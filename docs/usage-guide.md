...
이 문서는 격리 서버 하나를 controller로 두고, 여러 외부 서버를 SSH worker로
사용해 Hugging Face 모델을 병렬 다운로드하는 운영 절차입니다.
...
- controller: `uv run msdl ...`을 실행하는 중심 서버입니다. 보통 격리 서버가
  이 역할을 맡습니다.
- worker: 인터넷으로 Hugging Face에 접근 가능한 외부 서버입니다. 별도 서비스는
  돌리지 않고, controller가 SSH로 명령을 실행합니다.
...
```text
worker temp file
  -> controller .msdl/incoming/<job_id>/<server>/<path>.part
  -> size check
  -> final path로 rename
```
...
1. worker별 SSH 접속과 `hf`, `rsync`, `python3` 확인
2. `servers.toml` 작성
