...
이 문서는 controller 하나를 두고, 여러 Linux/Windows 서버를 SSH worker로 사용해
Hugging Face 모델을 병렬 다운로드하는 운영 절차입니다. 최종 목적지가 Linux
서버라면 그 Linux 서버에서 `msdl`을 실행하고, Windows PC는 worker로 등록합니다.
...
- controller: `uv run msdl ...`을 실행하는 중심 서버이자 최종 저장소입니다.
  최종 파일을 Linux 서버에 두려면 이 Linux 서버가 controller입니다.
- worker: 인터넷으로 Hugging Face에 접근 가능한 Linux 또는 Windows 서버입니다.
  별도 서비스는 돌리지 않고, controller가 SSH로 원격 명령을 실행합니다.
...
### 4.1 최종 Linux controller + Windows/Linux worker 구조
...
Hugging Face
  ...
  -> Linux worker 2 임시 디렉터리
  -> 최종 Linux controller가 각 파일을 pull
  -> /models/org/model/... 에 최종 저장
```

이 구조에서는 최종 Linux 서버에서 `msdl`을 실행합니다. Windows는 controller가
아니라 worker입니다.

최종 Linux controller 준비물:
...
- Linux worker -> Linux controller: `rsync`가 있으면 `rsync`
- Windows worker -> Linux controller: `scp`

중요한 경로 구분:

- `--save-path` 또는 `MULTISERVER_DOWNLOAD_SAVE_PATH`: Windows worker의
  경로가 아니라 최종 Linux controller의 로컬 경로입니다. 예: `/models`
...
- Linux worker의 `auto`: controller에 `rsync`가 있으면 `rsync`, 없으면 `scp`
- Windows worker의 `auto`: `scp`
